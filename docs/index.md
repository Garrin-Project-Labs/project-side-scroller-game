...
<body>
  ...
  <script>
    ...
    const ROBOT_H = 78;
    const GRAVITY = 0.54;
    const JUMP_POWER = -12.4;
    const MAX_JUMP_HEIGHT = 168;
    const MAX_HELD_JUMP_FRAMES = 60;
    const HELD_JUMP_GRAVITY_SCALE = 0.12;
    const OBSTACLE_GAP_PIXELS = 650;
    const BATTERY_GAP_PIXELS = 1120;
    const WATER_WIDTHS = [78, 96, 84, 116, 88];
    const BOX_HEIGHTS = [38, 46, 34];
    const BOX_SIZE = 44;
    const OBSTACLE_PATTERN = ['water', 'box', 'platform', 'stackedBox', 'water', 'box', 'water', 'platform'];
    ...
    class RunnerScene extends Phaser.Scene {
      ...
      spawnObstacle() {
        ...
        if (kind === 'water') {
          const w = this.obstaclePatternIndex === 1 ? 48 : WATER_WIDTHS[this.obstaclePatternIndex % WATER_WIDTHS.length];
      ...
      checkCollisions() {
        ...
        for (const o of this.obstacles) {
          if (o.kind === 'water') {
            if (this.overlap(hit, { x: o.x + 5, y: o.y - 5, w: o.w - 10, h: o.h + 10 })) this.splash();
          }
          if (o.kind === 'box' || o.kind === 'stackedBox') {
            if (this.overlap(hit, { x: o.x + 4, y: o.y + 4, w: o.w - 8, h: o.h - 4 })) this.splash();
          }
        }
        for (const p of this.pickups) {
      ...
      drawBackground() {
        this.bg.fillGradientStyle(0x17375e, 0x17375e, 0x244b74, 0x102039, 1).fillRect(0, 0, W, H);
        ...
        for (let x = -80 + ((this.tick * this.speed * 0.28) % 160); x < W + 200; x += 160) {
          this.bg.fillRect(x, 266, 70, 164).fillRect(x + 20, 230, 32, 36);
        }
        this.world.fillStyle(0x17283f, 1).fillRect(0, GROUND_Y, W, H - GROUND_Y);
        this.world.fillStyle(0x2bdf9f, 1).fillRect(0, GROUND_Y, W, 12);
      ...
      drawRobot() {
        ...
        const x = r.x + wobble;
        const y = r.y;
        g.fillStyle(0xb9d4e9, 1).fillRoundedRect(x + 10, y + 20, 38, 42, 10);
        g.fillStyle(0xdff8ff, 1).fillRoundedRect(x + 5, y, 48, 34, 11);
        g.fillStyle(0x112033, 1).fillRoundedRect(x + 13, y + 9, 32, 14, 7);
        g.fillStyle(r.blink < 6 ? 0x112033 : 0x64f4ff, 1).fillCircle(x + 23, y + 16, 3.5).fillCircle(x + 36, y + 16, 3.5);
        g.lineStyle(4, 0x64f4ff, 1).lineBetween(x + 29, y, x + 29, y - 11);
        g.fillStyle(0xffd95a, 1).fillCircle(x + 29, y - 14, 5);
        const leg = r.grounded ? Math.sin(this.tick * 0.32) * 7 : -4;
        g.lineStyle(8, 0xdff8ff, 1).lineBetween(x + 18, y + 60, x + 14 + leg, y + 76).lineBetween(x + 40, y + 60, x + 44 - leg, y + 76);
        g.lineStyle(8, 0x9fd8ff, 1).lineBetween(x + 10, y + 32, x, y + 46 + Math.sin(this.tick * 0.25) * 4).lineBetween(x + 48, y + 32, x + 58, y + 46 - Math.sin(this.tick * 0.25) * 4);
      }
      ...
      drawWater(o) {
        ...
        g.beginPath();
        g.moveTo(o.x, o.y).lineTo(o.x + o.w, o.y).lineTo(o.x + o.w - 12, o.y + o.h).lineTo(o.x + 12, o.y + o.h).closePath().fillPath();
        g.fillGradientStyle(0x83f5ff, 0x83f5ff, 0x174dc8, 0x174dc8, 1);
        g.beginPath();
        g.moveTo(o.x + 10, o.y + 16).lineTo(o.x + o.w - 10, o.y + 16).lineTo(o.x + o.w - 20, o.y + o.h - 6).lineTo(o.x + 20, o.y + o.h - 6).closePath().fillPath();
        ...
        for (let x = 14; x < o.w - 20; x += 18) {
          ...
          g.moveTo(o.x + x, sy).quadraticCurveTo(o.x + x + 8, o.y + 8, o.x + x + 16, ey);
        }
        g.strokePath();
        g.lineStyle(6, 0x2bdf9f, 1).lineBetween(o.x - 4, o.y + 1, o.x + 10, o.y + 1).lineBetween(o.x + o.w - 10, o.y + 1, o.x + o.w + 4, o.y + 1);
      }
      ...
      drawBox(o) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          g.fillGradientStyle(0xd9a04f, 0xd9a04f, 0x8a5328, 0x8a5328, 1).fillRoundedRect(o.x, y, o.w, blockH, 6);
          g.lineStyle(4, 0x5b351c, 1).strokeRoundedRect(o.x + 2, y + 2, o.w - 4, blockH - 4, 5);
          g.lineStyle(3, 0xffffff, 0.26).lineBetween(o.x + 10, y + 12, o.x + o.w - 10, y + blockH - 10).lineBetween(o.x + o.w - 10, y + 12, o.x + 10, y + blockH - 10);
        }
      }
      ...
      drawPlatform(o) {
        ...
        g.beginPath();
        g.moveTo(o.x, o.y + o.h).lineTo(o.x, o.y + 30).quadraticCurveTo(o.x, o.y + 8, o.x + 24, o.y + 8).lineTo(o.x + o.w - 24, o.y + 8).quadraticCurveTo(o.x + o.w, o.y + 8, o.x + o.w, o.y + 30).lineTo(o.x + o.w, o.y + o.h).closePath().fillPath();
        g.fillGradientStyle(0x68ffc7, 0x68ffc7, 0x2bdf9f, 0x2bdf9f, 1);
        g.beginPath();
        g.moveTo(o.x, o.y + 24).quadraticCurveTo(o.x, o.y, o.x + 26, o.y).lineTo(o.x + o.w - 26, o.y).quadraticCurveTo(o.x + o.w, o.y, o.x + o.w, o.y + 24).lineTo(o.x + o.w, o.y + 28).lineTo(o.x, o.y + 28).closePath().fillPath();
      ...
      drawBattery(p) {
        ...
        const y = p.y + Math.sin(p.bob) * 8;
        const g = this.world;
        g.fillStyle(0x273142, 1).fillRoundedRect(p.x, y + 6, p.w, p.h - 6, 7);
        g.fillStyle(0xffd95a, 1).fillRoundedRect(p.x + 7, y + 13, p.w - 14, p.h - 20, 5);
        g.fillStyle(0x7dff8a, 1).fillRect(p.x + 11, y + 20, p.w - 22, 8).fillRect(p.x + 11, y + 32, p.w - 22, 8);
        g.fillStyle(0xe8f3ff, 1).fillRoundedRect(p.x + 10, y, p.w - 20, 8, 3);
      }

      drawSparks() {
        for (const s of this.sparks) this.effects.fillStyle(s.color, Math.max(0, s.life / 36)).fillCircle(s.x, s.y, 3.5);
      }
